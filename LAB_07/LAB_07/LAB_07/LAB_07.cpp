...
//######################### FOR BEZIER CAMERA MOVEMENT ####################################

#define MaxNumPts 2000 //number of maximum points in a curve
#define Steps 200 //steps of the curve (number of points that it will have)

float PointArray[MaxNumPts][3] = { {15,0,0}, {13,3,13}, {0,0,15}, {-13,-3,13}, {-15,0,0}, {-13,3,-13 }, { 0,0,-15 }, {13,-3,-13,}, {15, 0, 0}}; //array of control points
float CurveArray[MaxNumPts][3]; //array of curve points
int NumPts = 9; //number of control points.
int curveStep = 0; //current step of curve for moving along it
...
void init_brick_column() {
	...
	objects.push_back(obj);
	movables.push_back(objects.size() - 1);
	obj.shading = ShadingType::TEXTURE_PHONG; // TEXTURE_PHONG; // TODO NORMAL_MAPPING;
...
void init_skybox() {
	...
	obj.M = glm::scale(glm::translate(glm::mat4(1), glm::vec3(5, 5, 5)), glm::vec3(100., 100., 100.));
	objects.push_back(obj);
}
...
void init_windows() {
	...
	obj.blended = false; // TODO false -> object solid; true -> object semi-transparent if Blending enabled;
	objects.push_back(obj);
	movables.push_back(objects.size() - 1);
...
void init_torus() {
	...
	obj.name = "Torus";
	//obj.diffuseTexID = generateCheckTexture();
	obj.diffuseTexID = loadTexture(TextureDir + "bombolone_2.jpg");
...
void drawScene() {
	...
	static vector<Object> transparent_objects; //vector of transparent objects

	for (int i = 0; i < objects.size(); i++) {
		// TODO if you handle transparency then you must jump any semi transparent object
		if (objects[i].blended == false) {
			//Shader selection
			...
			transparent_objects.push_back(objects[i]);
		}
	}
	...
	sortObjectsByDistance(transparent_objects);

	for (int i = 0; i < transparent_objects.size(); i++) {
		//Shader selection
		switch (transparent_objects[i].shading) {
		case ShadingType::NORMAL_MAPPING:
			glUseProgram(shaders_IDs[NORMAL_MAPPING]);
			// Caricamento matrice trasformazione del modello
			glUniformMatrix4fv(base_uniforms[NORMAL_MAPPING].M_Matrix_pointer, 1, GL_FALSE, value_ptr(transparent_objects[i].M));
			//Material loading
			glUniform3fv(light_uniforms[NORMAL_MAPPING].material_ambient, 1, glm::value_ptr(materials[transparent_objects[i].material].ambient));
			glUniform3fv(light_uniforms[NORMAL_MAPPING].material_diffuse, 1, glm::value_ptr(materials[transparent_objects[i].material].diffuse));
			glUniform3fv(light_uniforms[NORMAL_MAPPING].material_specular, 1, glm::value_ptr(materials[transparent_objects[i].material].specular));
			glUniform1f(light_uniforms[NORMAL_MAPPING].material_shininess, materials[transparent_objects[i].material].shininess);
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, transparent_objects[i].diffuseTexID);
			glActiveTexture(GL_TEXTURE1);
			glBindTexture(GL_TEXTURE_2D, transparent_objects[i].normalTexID);
			break;
		case ShadingType::TEXTURE_PHONG:
			glUseProgram(shaders_IDs[TEXTURE_PHONG]);
			// Caricamento matrice trasformazione del modello
			glUniformMatrix4fv(base_uniforms[TEXTURE_PHONG].M_Matrix_pointer, 1, GL_FALSE, value_ptr(transparent_objects[i].M));
			//Material loading
			glUniform3fv(light_uniforms[TEXTURE_PHONG].material_ambient, 1, glm::value_ptr(materials[transparent_objects[i].material].ambient));
			glUniform3fv(light_uniforms[TEXTURE_PHONG].material_diffuse, 1, glm::value_ptr(materials[transparent_objects[i].material].diffuse));
			glUniform3fv(light_uniforms[TEXTURE_PHONG].material_specular, 1, glm::value_ptr(materials[transparent_objects[i].material].specular));
			glUniform1f(light_uniforms[TEXTURE_PHONG].material_shininess, materials[transparent_objects[i].material].shininess);
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, transparent_objects[i].diffuseTexID);
			break;
		case ShadingType::SKYBOX:
			glUseProgram(shaders_IDs[SKYBOX]);
			// Caricamento matrice trasformazione del modello
			glUniformMatrix4fv(base_uniforms[SKYBOX].M_Matrix_pointer, 1, GL_FALSE, value_ptr(transparent_objects[i].M));
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_CUBE_MAP, transparent_objects[i].diffuseTexID);
			// We draw the skybox first, so if we disable writes on the Z-BUFFER, 
			// all later draw calls will overwrite the skybox pixels for sure, 
			// no matter how distant is the skybox.
			glDepthMask(GL_FALSE);
		...
		case ShadingType::REFLECTION:
			glUseProgram(shaders_IDs[REFLECTION]);
			// Caricamento matrice trasformazione del modello
			glUniformMatrix4fv(base_uniforms[REFLECTION].M_Matrix_pointer, 1, GL_FALSE, value_ptr(transparent_objects[i].M));
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_CUBE_MAP, transparent_objects[i].diffuseTexID);
			break;
		case ShadingType::REFRACTION:
			glUseProgram(shaders_IDs[REFRACTION]);
			// Caricamento matrice trasformazione del modello
			glUniformMatrix4fv(base_uniforms[REFRACTION].M_Matrix_pointer, 1, GL_FALSE, value_ptr(transparent_objects[i].M));
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_CUBE_MAP, transparent_objects[i].diffuseTexID);
			break;
		case ShadingType::TEXTURE_ONLY:
			glUseProgram(shaders_IDs[TEXTURE_ONLY]);
			// Caricamento matrice trasformazione del modello
			glUniformMatrix4fv(base_uniforms[TEXTURE_ONLY].M_Matrix_pointer, 1, GL_FALSE, value_ptr(transparent_objects[i].M));
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, transparent_objects[i].diffuseTexID);
			break;
		case ShadingType::PASS_THROUGH:
			glUseProgram(shaders_IDs[PASS_THROUGH]);
			// Caricamento matrice trasformazione del modello
			glUniformMatrix4fv(base_uniforms[PASS_THROUGH].M_Matrix_pointer, 1, GL_FALSE, value_ptr(transparent_objects[i].M));
			break;
		default:
			break;
		}
		...
		glBindVertexArray(transparent_objects[i].mesh.vertexArrayObjID);
		glDrawArrays(GL_TRIANGLES, 0, transparent_objects[i].mesh.vertices.size());
...
void computeTorusVertex(int i, int j, Mesh* mesh) {
	...
	mesh->texCoords.push_back(glm::vec2(phi / (2*pig), theta / (2 * pig)));
	}
